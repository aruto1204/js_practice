...
// 抽象クラス: インスタンス化できないが、継承の基底クラスとして機能する
...
// 抽象クラスでもコンストラクタを定義できる（派生クラスで super() を呼び出す）
...
// テンプレートメソッドパターン: 処理の流れは抽象クラスで定義し、詳細は派生クラスで実装
...
// 抽象クラスの階層構造: 抽象クラスが別の抽象クラスを継承できる
...
// ジェネリクスを使った抽象クラス: 型パラメータで再利用可能な設計
...
// protected な抽象メソッド: 派生クラス内部でのみ使用される処理を強制
...
// Factory パターン: オブジェクトの生成ロジックをカプセル化
...
// 状態管理パターン: React のステートフルコンポーネントのような設計
...
// ビルダーパターン: メソッドチェーンで段階的にオブジェクトを構築
...
// Command パターン: 操作をオブジェクトとしてカプセル化し、undo/redo を実現
