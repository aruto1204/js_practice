...
// implements キーワードを使用してインターフェースを実装
// クラスは implements したインターフェースのすべてのメンバーを実装する必要がある
interface Printable {
  print(): void;
}
...
// インターフェースはプロパティとメソッドの両方を定義できる
// 実装クラスは定義されたすべてのプロパティとメソッドを持つ必要がある
interface Person {
  name: string;
  age: number;
  greet(): string;
...
// オプショナルメンバーは ? を使って定義する
// オプショナルメンバーは実装クラスで実装してもしなくてもよい
interface Config {
  apiUrl: string;
  timeout?: number;
  validate(): boolean;
...
// ジェネリックインターフェースは型パラメータを持つ
// 実装クラスもジェネリックにして型パラメータを受け取る
...
// 継承されたインターフェースのメンバーもすべて実装する必要がある
interface Animal {
  makeSound(): string;
}
...
// インターフェースは関数型のプロパティを定義できる
// 関数型プロパティはコールバック関数などで使用される
...
// 抽象クラスでもインターフェースを実装できる
// 抽象クラスはインターフェースのメンバーを抽象メソッドとして宣言することもできる
...
// インターフェースは非同期メソッドも定義できる
// ジェネリックメソッドを持つインターフェースも実装可能
interface Database {
...
// インターフェースは他のインターフェース型のプロパティを持つことができる
// これにより依存性注入のような設計パターンを実現できる
interface Logger {
...
// インターフェースは静的メンバーを定義できない
// インスタンスメソッドとして定義し、静的メソッドは別途実装する
...
// readonly 修飾子を使用して読み取り専用プロパティを定義できる
// 実装クラスでも readonly として実装する必要がある
...
// インデックスシグネチャを持つインターフェースを実装できる
// インデックスシグネチャにより動的なプロパティアクセスが可能になる
interface StringMap {
...
// インターフェースを実装したクラスを継承することができる
// 継承クラスでメソッドをオーバーライドできる
