...
class Employee {
  ...
  // 連絡先情報を返すメソッド（電話番号がある場合のみ含める）
  getContact(): string {
    let contact = `名前: ${this.name}, 部署: ${this.department}`;
    if (this.phoneNumber) {
      contact += `, 電話: ${this.phoneNumber}`;
    }
    return contact;
  }
}
...
// パラメータプロパティを使用した簡潔な実装
class Point {
  // コンストラクタのパラメータに public を付けることで、自動的にプロパティとして定義される
...
// メソッドチェーンのパターンを実装（各メソッドが this を返す）
class Calculator {
...
// シングルトンパターンの実装（インスタンスが1つだけ存在することを保証）
